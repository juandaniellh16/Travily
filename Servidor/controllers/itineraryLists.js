...
import { validateItineraryList, validatePartialItineraryList } from '../schemas/itineraryLists.js'
...
export class ItineraryListController {
  ...
  getAll = async (req, res, next) => {
    ...
    const { userId, username, likedBy, visibility = 'public', sort, limit = 10 } = req.query
    try {
      if (userId && username) {
        throw new InvalidInputError('You cannot filter by userId and username at the same time')
      }
      if (visibility !== 'public' && !user) {
        throw new UnauthorizedError('You are not authorized to view private itinerary lists')
      }
      const limitValue = parseInt(limit, 10)
      const lists = await this.itineraryListModel.getAll({ userId, username, likedBy, visibility, sort, limit: limitValue, userIdSession: user?.id })
      res.json(lists)
    } catch (error) {
      next(error)
    }
  ...
  create = async (req, res, next) => {
    ...
    try {
      ...
      if (!result.success) {
        throw new InvalidInputError('Invalid itinerary list data: ' + JSON.stringify(result.error.message))
      }

      if (user.id !== req.body.userId) {
        throw new UnauthorizedError('You are not authorized to create an itinerary list for another user')
      }

      const listId = await this.itineraryListModel.create({ input: result.data })
  ...
  like = async (req, res, next) => {
    ...
    try {
      if (!user) throw new UnauthorizedError('Access not authorized')
      if (!listId) throw new InvalidInputError('Itinerary list id parameter is required')
  ...
  checkIfLiked = async (req, res, next) => {
    ...
    try {
      ...
      const result = await this.itineraryListModel.checkIfLiked({ listId, userId })
  ...
  removeItineraryFromList = async (req, res, next) => {
    ...
    try {
      ...
      await this.itineraryListModel.removeItineraryFromList({ listId, itineraryId })
      res.status(204).end()
    } catch (error) {
      next(error)
    }
